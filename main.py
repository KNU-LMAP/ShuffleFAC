...
def main():
    ...
    # Model
    model = CRNN(**crnn_cfg)
    macs, _ = calculate_macs(model, configs)
    ...
    for epoch in range(num_epochs):
        ...
        val_loss, val_acc, val_f1 = valid(model, val_loader, criterion, device)
        ...
        if val_f1 > best_f1:
            best_f1 = val_f1
            torch.save(
                {'epoch': epoch + 1, 'model_state': model.state_dict(), 'best_f1': best_f1},
    ...
    test_loss, test_acc, test_macro_f1 = test(model, test_loader, criterion, device)
