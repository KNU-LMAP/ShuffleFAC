...
#     encoder,
#     thresholds=[0.5],
#     median_filter= [5,11,5,5,5,67,61,49,5,17],
#     pad_indx=None,
# ):
...
def batched_decode_preds(
    ...
    encoder,
    thresholds=[0.5],
    median_filter=7,  # int or list[int] (len == num_classes)
    pad_indx=None,
):
    import pandas as pd
    import numpy as np
    ...
    for j in range(strong_preds.shape[0]):  # over batches
        audio_id = Path(filenames[j]).stem
        filename = audio_id + ".wav"
        c_scores = strong_preds[j]  # shape: [C, T]

        if pad_indx is not None:
            true_len = int(c_scores.shape[-1] * pad_indx[j].item())
            c_scores = c_scores[:, :true_len]

        c_scores = c_scores.transpose(0, 1).detach().cpu().numpy()  # shape: [T, C]

        # 저장: 필터 적용 전 raw score
        scores_raw[audio_id] = create_score_dataframe(
            scores=c_scores,
            timestamps=encoder._frame_to_time(np.arange(len(c_scores) + 1)),
            event_classes=encoder.labels,
        ...
        # 클래스별 median filter 적용
        filtered_scores = np.empty_like(c_scores)
        for class_idx in range(len(encoder.labels)):
            if isinstance(median_filter, list):
                filt_width = median_filter[class_idx]
            else:
                filt_width = median_filter
            filtered_scores[:, class_idx] = scipy.ndimage.median_filter(
                c_scores[:, class_idx], size=filt_width
            )

        # 저장: 필터 적용 후 score
        scores_postprocessed[audio_id] = create_score_dataframe(
            scores=filtered_scores,
            timestamps=encoder._frame_to_time(np.arange(len(filtered_scores) + 1)),
            event_classes=encoder.labels,
        )

        for c_th in thresholds:
            pred = filtered_scores > c_th
            pred = encoder.decode_strong(pred)
            pred = pd.DataFrame(pred, columns=["event_label", "onset", "offset"])
            pred["filename"] = filename
            prediction_dfs[c_th] = pd.concat(
